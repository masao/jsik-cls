...
* 参考: [投稿者の方へ - 情報知識学会](https://www.jsik.jp/?tokokitei)
...
タイトルページ内の所属、キーワード等には、本文より小さい `\small` を使用しています。

11pt 文書における `\small` は、おおむね 10pt 相当で、執筆要領にあわせたものです。

### 最終ページ

`balance` パッケージによる段組均等化は、図表、脚注、長い参考文献等がある場合に必ずしも理想的な結果になるとは限りません。

標準設定で最終ページ段組みが均等にならない場合は、`balance`パッケージが読み込まれているか確認したり、必要に応じて本文末尾付近に直接 `\balance` コマンドを追加したりしてみてください。
